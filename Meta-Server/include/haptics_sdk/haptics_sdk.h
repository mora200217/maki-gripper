...
// 
// The Haptics SDK uses OpenXR for communicating with Quest controllers.
//
// To initialize the Haptics SDK with OpenXR, perform the following steps in order.
// Performing the initialization steps in the wrong order will most likely result in errors.
//
// 1. When you create the OpenXR instance with xrCreateInstance(), include the extensions needed by
//    the SDK.
//   a. You can get the names of the required extensions with
//      haptics_sdk_get_openxr_extension_count() and haptics_sdk_get_openxr_extension().
// 2. After creating the OpenXR instance, pass it to the SDK by calling
//    haptics_sdk_set_openxr_instance().
// 3. After creating the OpenXR session, pass it to the SDK by calling
//    haptics_sdk_set_openxr_session().
// 4. When you set up your OpenXR actions, pass an action set to the SDK by calling
...
// 5. Bind the output action by including the SDK’s suggested bindings in your call to
...
// 6. Attach the action set to the OpenXR session with xrAttachSessionActionSets().
// 7. To save power when the system is idle, whenever the OpenXR session state changes,
...
/**
 * Puts the number of OpenXR extensions used by the Haptics SDK into the extension_count_out parameter.
 *
 * After getting this count, haptics_sdk_get_openxr_extension() should be called repeatedly to
 * retrieve the required extension names.
 *
...
/**
 * Returns a pointer to one of the required OpenXR extensions
 *
 * The number of required extensions should be retrieved by calling
 ...
 *
 * Include the Haptics SDK's required extensions when calling xrCreateInstance().
 *
...
/**
 * Sets the OpenXR instance to be used by the Haptics SDK
 *
 * Before setting the OpenXR instance, appropriate OpenXR extensions must be enabled. The list of
 * required extensions can be retrieved via calls to haptics_sdk_get_openxr_extension().
 *
 * After setting the instance, playback operations won't be available until
 ...
 *   haptics_sdk_set_openxr_session()
 */
HapticsSdkResult haptics_sdk_set_openxr_instance(XrInstance openxr_instance);

/**
 * Sets the OpenXR session to be used by the Haptics SDK
 *
 * This must be called after setting the OpenXR instance via haptics_sdk_set_openxr_instance().
 *
